	...
	class TypeMappingNativeAssemblyGenerator : NativeAssemblyGenerator
	{
		...
		void WriteManagedMaps<TKey> (StreamWriter output, string moduleSymbolName, IDictionary<TKey, TypeMapGenerator.TypeMapEntry> entries)
		{
			...
			foreach (var kvp in tokens) {
				uint token = kvp.Key;
				uint javaMapIndex = kvp.Value;

				size += WriteStructure (output, packed: false, structureWriter: () => WriteManagedMapEntry (output, token, javaMapIndex));
			}
		...
		void WriteMapModules (StreamWriter output, StreamWriter mapOutput, string symbolName)
		{
			...
			foreach (var kvp in mappingData.Modules) {
				...
				size += WriteStructure (output, packed: false, structureWriter: () => WriteMapModule (output, mapName, duplicateMapName, mvid, data));
				if (mapOutput != null) {
