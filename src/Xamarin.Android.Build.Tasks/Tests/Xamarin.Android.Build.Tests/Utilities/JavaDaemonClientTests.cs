...
{
	[TestFixture]
	class JavaDaemonClientTests : BaseTest
	{
		...
		JavaDaemonClient Connect ()
		{
			...
			client.Connect (java, $"-jar \"{java_daemon}\"");
			var processId = client.ProcessId;
			Assert.IsTrue (processId > 0, $"Invalid process id: {processId}");
...
build engineering";
		...
		public void R8Version ()
		{
			using (var client = Connect ()) {
				...
				Assert.AreEqual (Expected.R8Version, stdout.Trim ());
		...
		public void DifferentMainClass ()
		{
			using (var client = Connect ()) {
				...
				Assert.AreEqual (0, exitCode, $"Exited with {exitCode}: {stdout} {stderr}");
				Assert.AreEqual (Expected.D8Version, stdout.Trim ());
				Assert.AreEqual ("", stderr.Trim ());
			}
		}
		...
		public void DifferentJar ()
		{
			using (var client = Connect ()) {
				...
				Assert.AreEqual (0, exitCode, $"Exited with {exitCode}: {stdout} {stderr}");
				Assert.AreEqual (Expected.BundleToolVersion, stdout.Trim ());
				Assert.AreEqual ("", stderr.Trim ());
			}
		}
		...
		public void WorkingDirectory ()
		{
			...
			var tempFile = Path.Combine (tempPath, TestName + ".zip");
			try {
				File.WriteAllText (tempFile, contents: "");
				using (var client = Connect ()) {
					var relative = Path.GetFileName (tempFile);
					(int exitCode, string stdout, string stderr) = client.Invoke ("com.android.tools.build.bundletool.BundleToolMain", BundleToolPath, $"build-bundle --modules \"{relative}\" --output foo.aab" /*, workingDirectory: tempPath */);
					Assert.AreEqual (1, exitCode, $"Exited with {exitCode}: {stdout} {stderr}");
					Assert.AreEqual ("", stdout.Trim ());
					//FIXME: not working yet, WorkingDirectory not working on the Java side
					//StringAssert.Contains ("does not seem to be a valid ZIP file", stderr);
					StringAssert.Contains ($"Error: File '{relative}' was not found", stderr);
				}
			} finally {
