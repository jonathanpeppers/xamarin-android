...
const char* BasicAndroidSystem::running_on_abi_name = nullptr;
...
const char*
BasicAndroidSystem::get_running_on_abi_name ()
{
	if (running_on_abi_name == nullptr) {
		unsigned short built_for_cpu = 0, running_on_cpu = 0;
		unsigned char is64bit = 0;
		_monodroid_detect_cpu_and_architecture (&built_for_cpu, &running_on_cpu, &is64bit);
		running_on_abi_name = android_abi_names [running_on_cpu];
	}
	return running_on_abi_name;
}
