...
EmbeddedAssemblies::binary_search (const Key *key, const Entry *base, size_t nmemb, [[maybe_unused]] size_t extra_size)
{
	...
	if (base == nullptr) {
		...
		exit (FATAL_EXIT_MISSING_ASSEMBLY); // TODO: use a proper error code here
	}

	log_warn (LOG_ASSEMBLY, "binary_search: base == %p", base);
	if constexpr (use_extra_size) {
		log_warn (LOG_ASSEMBLY, "binary_search: extra_size == %u", extra_size);
	}

	constexpr size_t size = sizeof(Entry);
	while (nmemb > 0) {
		log_warn (LOG_ASSEMBLY, "binary_search: loop start, nmemb == %u", nmemb);
		const Entry *ret;
		if constexpr (use_extra_size) {
		...
		log_warn (LOG_ASSEMBLY, "binary_search: ret == %p", ret);

		int result = compare (key, ret);
		log_warn (LOG_ASSEMBLY, "binary_search: result == %d", result);
		if (result < 0) {
			nmemb /= 2;
		} else if (result > 0) {
			if constexpr (use_extra_size) {
...
EmbeddedAssemblies::typemap_java_to_managed (MonoString *java_type)
{
	...
	if (application_config.instant_run_enabled) {
		...
		if (entry == nullptr) {
			log_warn (LOG_ASSEMBLY, "typemap: unable to find mapping from Java type '%s' to managed type with token ID %u in module [%s]", java_type_name.get (), java_entry->type_token_id, mono_guid_to_string (module->module_uuid));
			return nullptr;
		}
...
{
	// Looking for cause of a segfault which occurs only on the build bots, the log entries will be removed later on
	if (entry == nullptr || entry->java_name[0] == '\0') {
		if (entry == nullptr)
			log_warn (LOG_ASSEMBLY, "compare_java_name: entry is nullptr");
		else
			log_warn (LOG_ASSEMBLY, "compare_java_name: entry (module: %u; token: 0x%x / %u) has an empty java_name", entry->module_index, entry->type_token_id, entry->type_token_id);

		return -1;
	}

	if (java_name == nullptr)
		log_warn (LOG_ASSEMBLY, "compare_java_name: java_name is nullptr");
	else
		log_warn (LOG_ASSEMBLY, "compare_java_name: java_name == '%s'", java_name);

	log_warn (LOG_ASSEMBLY, "compare_java_name: entry->java_name == '%s'", reinterpret_cast<const char*>(entry->java_name));
...
EmbeddedAssemblies::typemap_managed_to_java (const uint8_t *mvid, const int32_t token)
{
	...
	if (match == nullptr) {
		log_warn (LOG_ASSEMBLY, "typemap: module matching MVID [%s] not found.", mono_guid_to_string (mvid));
		return nullptr;
	}

	if (match->map == nullptr) {
		log_warn (LOG_ASSEMBLY, "typemap: module with MVID [%s] has no associated type map.", mono_guid_to_string (mvid));
		return nullptr;
	}
	...
	if (entry == nullptr) {
		if (match->duplicate_count > 0 && match->duplicate_map != nullptr) {
			log_debug (LOG_ASSEMBLY, "typemap: searching module [%s] duplicate map for token %u (0x%x)", mono_guid_to_string (mvid), token, token);
			entry = binary_search <int32_t, TypeMapModuleEntry, compare_type_token> (&token, match->duplicate_map, match->duplicate_count);
		}

		if (entry == nullptr) {
			log_warn (LOG_ASSEMBLY, "typemap: type with token %d (0x%x) in module {%s} (%s) not found.", token, token, mono_guid_to_string (mvid), match->assembly_name);
			return nullptr;
		}
	}
	...
	if (entry->java_map_index >= java_entry_count) {
		log_warn (LOG_ASSEMBLY, "typemap: type with token %d (0x%x) in module {%s} (%s) has invalid Java type index %u", token, token, mono_guid_to_string (mvid), match->assembly_name, entry->java_map_index);
		return nullptr;
	}
	...
	if (application_config.instant_run_enabled) {
		...
		ret = reinterpret_cast<char*>(const_cast<uint8_t*>(java_entry->java_name));
#if defined (DEBUG) || !defined (ANDROID)
	}
#endif
...
EmbeddedAssemblies::typemap_load_file (BinaryTypeMapHeader &header, const char *dir_path, const char *file_path, int file_fd, TypeMapModule &module)
{
	...
	log_debug (
		...
		mono_guid_to_string (header.module_uuid), header.assembly_name_length, module.assembly_name
	);
